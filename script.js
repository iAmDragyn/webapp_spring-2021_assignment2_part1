function searchGoogleBooks() {
    ...
    fetch('https://www.googleapis.com/books/v1/volumes?q=search+terms' + txtSearch.value + "&startIndex=0&maxResults=20")
        .then(response => response.json())
        .then(json => {
            var list = `<div class="row col-sm-5 text-center">found ${json.totalItems} books </div> 
        <div class="row">`
            for (let i = 0; i < json.items.length; i++) {
                ...
                template += `<a class="list-group-item list-group-item-action list-group-item-dark" href="${json.items[i].selfLink}"> ${bookInfo.title} </a>`
